...
// ── Card formatter ────────────────────────────────────────────────────────────
function formatCard(card, includeVectorScore = false) {
    const lines = [];
    ...
    const ci = parseJson(card.color_identity);
    if (ci.length > 0)
        lines.push(`Color Identity: ${ci.join("")}`);
    if (card.tags.length > 0) {
        lines.push(`Tags: ${card.tags.join(", ")}`);
    }
    if (card.edhrec_rank)
        lines.push(`EDHREC rank: #${card.edhrec_rank.toLocaleString()}`);
...
function formatCombo(combo) {
    ...
    if (combo.mana_needed)
        lines.push(`Mana needed: ${combo.mana_needed}`);
    if (combo.description)
        lines.push(`Steps: ${combo.description.trim()}`);
...
export function buildContext(result, intent) {
    const sections = [];
    let charCount = 0;
    let truncated = false;
    // ── Header ────────────────────────────────────────────────────────────────
    ...
    for (const combo of result.combos) {
        ...
        if (charCount + text.length > CHAR_BUDGET) {
            truncated = true;
            break;
        }
 ...
 */
export function buildDeckSystemBlock(deck) {
    const lines = [];
    ...
    for (const card of deck.cards) {
        if (card.section === "commander" || card.section === "companion")
            continue;
...
export function buildSystemPrompt(intent, mode = "succinct") {
