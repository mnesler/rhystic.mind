...
describe("buildContext", () => {
    ...
    it("includes vector score for deck-build intent", () => {
        ...
        const result = buildContext(makeResult({ cards: [card] }), makeIntent({ type: "deck-build" }));
        expect(result.text).toContain("Relevance: 87.6%");
    });
    it("does not include vector score for card-lookup intent", () => {
        const card = makeCard({ vectorScore: 0.876 });
        const result = buildContext(makeResult({ cards: [card] }), makeIntent({ type: "card-lookup" }));
    ...
    it("places combos before cards in output", () => {
        const result = buildContext(makeResult({ cards: [makeCard()], combos: [makeCombo()] }), makeIntent());
    ...
    it("truncates and sets truncated=true when budget is exceeded", () => {
        ...
        const cards = Array.from({ length: 10 }, (_, i) => makeCard({ oracle_id: `id-${i}`, name: `Card ${i}`, oracle_text: bigOracleText }));
    ...
    it("does not truncate when content fits within budget", () => {
        const cards = Array.from({ length: 3 }, (_, i) => makeCard({ oracle_id: `id-${i}`, name: `Card ${i}` }));
        const result = buildContext(makeResult({ cards }), makeIntent());
        expect(result.truncated).toBe(false);
