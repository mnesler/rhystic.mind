...
import { createSession, addUserMessage, addAssistantMessage, } from "./conversation.js";
// ── ANSI colours ──────────────────────────────────────────────────────────────
const RESET = "\x1b[0m";
const BOLD = "\x1b[1m";
const DIM = "\x1b[2m";
...
// ── Main ──────────────────────────────────────────────────────────────────────
async function runTurn(message, session) {
    ...
    try {
        ...
        if (intent.commander)
            process.stdout.write(` ${dim("| Commander:")} ${cyan(intent.commander)}`);
        if (intent.tags.length > 0)
            process.stdout.write(` ${dim("| Tags:")} ${cyan(intent.tags.join(", "))}`);
    ...
    try {
        ...
        process.stdout.write(`\r  ${dim("Retrieved:")} ${cyan(String(result.cards.length))} cards, ${cyan(String(result.combos.length))} combos\n`);
        if (!result.hasEmbeddings) {
            process.stdout.write(yellow("  [no embeddings] Semantic search unavailable. Run `npm run embed:cards` for better results.\n"));
        }
    }
    catch (err) {
    ...
    let fullText = "";
    await streamAnswer(systemPrompt, context, session.history.slice(0, -1), message, {
        onToken: (token) => {
