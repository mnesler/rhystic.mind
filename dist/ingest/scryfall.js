...
function resolveOracleText(card) {
    if (card.oracle_text)
...
async function main() {
    ...
    if (!entry)
        throw new Error("oracle_cards bulk entry not found in Scryfall response");
    ...
    const filtered = cards.filter((c) => {
        if (c.legalities?.commander !== "legal")
            return false;
        if (setCode && c.set.toLowerCase() !== setCode)
            return false;
        return true;
    });
    console.log(`Commander-legal cards${setCode ? ` in set '${setCode}'` : ""}: ${filtered.length.toLocaleString()}`);
    // ── Upsert ────────────────────────────────────────────────────────────────
