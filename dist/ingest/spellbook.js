...
function identityToArray(identity) {
...
async function main() {
  ...
  `);
    ...
    const existing = db.prepare("SELECT COUNT(*) as n FROM combos").get().n;
    const startOffset = existing;
    let url = `${BASE_URL}?format=json&limit=${PAGE_SIZE}&offset=${startOffset}`;
  ...
  `);
    ...
    const unresolved = db
        ...
        .get();
    if (unresolved.n > 0) {
        console.log(`Note: ${unresolved.n} combo_cards rows still have no oracle_id ` +
