...
import { buildSystemPrompt, buildUserPrompt, parseTagsFromResponse, } from "./prompt.js";
// ── Config ────────────────────────────────────────────────────────────────────
...
async function callLLM(card) {
    const apiKey = process.env.OPEN_ROUTER_KEY ??
        ...
        "";
    if (!apiKey) {
        throw new Error("No OpenRouter API key found. Set OPEN_ROUTER_KEY environment variable.");
    }
    const body = {
        model: DEFAULT_MODEL,
        messages: [
...
async function main() {
    ...
    const cards = db
      ...
      LIMIT ?
    `)
        .all(setCode, batchSize);
    if (cards.length === 0) {
        // All cards in this set are tagged
        if (runId) {
            db.prepare(`UPDATE tagging_runs SET status = 'completed', completed_at = datetime('now') WHERE id = ?`).run(runId);
        }
    ...
    if (runId) {
        db.prepare(`INSERT OR IGNORE INTO tagging_runs (id, set_code) VALUES (?, ?)`).run(runId, setCode);
    }
    const insertTag = db.prepare(`INSERT OR IGNORE INTO card_tags (oracle_id, tag, source) VALUES (?, ?, 'llm')`);
    function insertTags(oracleId, tags) {
        db.exec("BEGIN");
        try {
            for (const tag of tags)
                insertTag.run(oracleId, tag);
            db.exec("COMMIT");
        }
        catch (err) {
            db.exec("ROLLBACK");
    ...
    for (const card of cards) {
        try {
            ...
            process.stderr.write(`  [${tagged}/${cards.length}] ${card.name}: ${tags.join(", ")}\n`);
        }
        catch (err) {
            // On LLM error: write needs-review and continue — don't fail the whole batch
            process.stderr.write(`  ERROR tagging ${card.name}: ${err}\n`);
    ...
    // Update run stats
    if (runId) {
        db.prepare(`UPDATE tagging_runs SET cards_tagged = cards_tagged + ? WHERE id = ?`).run(tagged, runId);
    }
...
//# sourceMappingURL=tag_set.js.map