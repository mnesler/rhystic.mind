...
describe("fetchMoxfieldDeck", () => {
    ...
    it("throws MoxfieldError for invalid URL", async () => {
        await expect(fetchMoxfieldDeck("https://archidekt.com/decks/123")).rejects.toThrow(MoxfieldError);
        await expect(fetchMoxfieldDeck("https://archidekt.com/decks/123")).rejects.toThrow("Invalid Moxfield URL");
    });
...
describe("Moxfield response normalization", () => {
    it("extracts commanders from the commanders field", () => {
        const commanders = Object.values(MOCK_DECK_RESPONSE.commanders).map((e) => e.card.name);
        expect(commanders).toEqual(["Kinnan, Bonder Prodigy"]);
    });
    it("sums mainboard quantities for cardCount", () => {
        const count = Object.values(MOCK_DECK_RESPONSE.mainboard).reduce((sum, e) => sum + e.quantity, 0);
...
//# sourceMappingURL=moxfield.test.js.map